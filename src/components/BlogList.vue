<template>
  ...
  <div v-for="article in info.results" v-bind:key="article.url" id="articles" class="row g-0 border rounded overflow-hidden flex-md-row mb-4 shadow-sm h-md-250 position-relative">
    <div class="col-auto d-none d-lg-block">
      <img v-if="article.avatar" :src="imageIfExists(article)" class="bd-placeholder-img" width="300" height="250" role="img" aria-label="Placeholder: Thumbnail" preserveAspectRatio="xMidYMid slice" focusable="false" alt="Thumbnail">
      <img v-else src="avatat404.jpg" class="bd-placeholder-img" width="300" height="250" role="img" aria-label="Placeholder: Thumbnail" preserveAspectRatio="xMidYMid slice" focusable="false" alt="Thumbnail">
    </div>
    <div class="col p-4 d-flex flex-column position-static">
      <strong v-if="article.category !== null" class="d-inline-block mb-2 text-primary">{{article.category.title}}</strong>
      <h3 class="mb-0">
          {{ article.title }}
      </h3>
